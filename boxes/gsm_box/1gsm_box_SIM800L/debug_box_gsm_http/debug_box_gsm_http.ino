...
void parse(String input){
  int indexQuotationMarkDirect = input.indexOf("{");
  int indexQuotationMarkRevers = input.indexOf("}");
  if (indexQuotationMarkDirect == -1 || indexQuotationMarkRevers == -1) return;
  String jsonString = input.substring(indexQuotationMarkDirect, indexQuotationMarkRevers + 1);
...
bool setBoxIdFile(){
  ...
  updateSerial();
  Serial2.println("AT+HTTPSSL=0");//Set PDP parameter
  updateSerial();
  ...
  updateSerial();
  Serial2.println("AT+HTTPPARA=\"USERDATA\",\"secret-key:a086d0ee0aff004b5034fcdb04ec400c\"");//Set PDP parameter
  updateSerial();
  Serial2.println("AT+HTTPPARA=\"URL\",\"http://box-dev.dvlb.ru/app/boxes/box/activate\"");//Get local IP address
  updateSerial();
  Serial2.println("AT+HTTPACTION=0");// Connect to the server then the server will send back former data
  delay(2000);
...
void loop(void) {
  ...
  // String data = "{\"box_id\":\"175\", \"mark_id\":\"111222333\", \"event_time\":\"1706888108\", \"secret_key\":\"a086d0ee0aff004b5034fcdb04ec400c\"}";
  // Serial.println("data = ");
  // Serial.println(data);
  // sendToGSM(data, 1);
...
void sim_card_setup(){
  ...
  do{
    ...
    Serial.println(RespCodeStr);
    simCardFail++;
  } while (!(RespCodeStr.indexOf("+CREG: 1,1") >= 0) && !(RespCodeStr.indexOf("+CREG: 0,1") >= 0));

  Serial2.println("AT+SAPBR=3,1,\"Contype\",\"GPRS\"");
  updateSerial();
  #if SIM_MODE == MTS
    Serial2.println("AT+SAPBR=3,1,\"APN\",\"internet.mts.ru\"");
    updateSerial();
    Serial2.println("AT+SAPBR=3,1,USER,\"mts\"");
    updateSerial();
    Serial2.println("AT+SAPBR=3,1,PWD,\"mts\"");
    updateSerial();
  #else
    Serial2.println("AT+SAPBR=3,1,\"APN\",\"Public.MC\"");
    updateSerial();
    Serial2.println("AT+SAPBR=3,1,USER,\"gdata\"");
    updateSerial();
    Serial2.println("AT+SAPBR=3,1,PWD,\"gdata\"");
    updateSerial();
  #endif
...
void parseToTime(String input){
  int indexUnixTime = input.indexOf("OK");
  String unixTime = input.substring(indexUnixTime - 12, indexUnixTime - 2);
...
void setTime(){
  ...
  Serial2.println("AT+HTTPPARA=\"CID\",\"1\"");//Set PDP parameter
  updateSerial();
  Serial2.println("AT+HTTPSSL=0");
  updateSerial();
  Serial2.println("AT+HTTPPARA=\"CONTENT\",\"application/json\"");//Activate PDP; Internet connection is available after successful PDP activation
  updateSerial();
  Serial2.println("AT+HTTPPARA=\"URL\",\"http://box-dev.dvlb.ru/app/timeUnix\"");// Connect to the server then the server will send back former data
  updateSerial();
...
bool sendToGSM(String data, bool ledOn){
  ...
  updateSerial();
  Serial2.println("AT+HTTPDATA=" + String(data.length()) + ",2000");// Connect to the server then the server will send back former data
  updateSerial();
  ...
  Serial.print("fail_send     = ");
  Serial.println(fail_send);
