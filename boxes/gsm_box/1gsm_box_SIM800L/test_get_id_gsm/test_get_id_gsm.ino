...
void sim_card_setup(){
  ...
  do{
    ...
    simCardFail++;
  } while (!(RespCodeStr.indexOf("+CREG: 0,1") >= 0));
...
bool setBoxIdFile(){
  ...
  updateSerial();
  Serial2.println("AT+HTTPPOST=\"URL\",\"https://box-dev.dvlb.ru/server/boxes/activate\",\"{\"secret_key\":\"a086d0ee0aff004b5034fcdb04ec400c\"}\"");//Get local IP address
  updateSerial();
...
void setRootCA(){
  Serial2.println("AT+SD2PCM=0");
  updateSerial();
  Serial2.println("AT&W");
  updateSerial();
  rebootGSM();

  Serial2.println("AT+FSCREATE=/home/anton202/devlab/boxes/gsm_box/1gsm_box_SIM800L/test_get_id_gsm/root_crt.pem<CR><LF>");
  updateSerial();
  Serial2.println("AT+FSWRITE=/home/anton202/devlab/boxes/gsm_box/1gsm_box_SIM800L/test_get_id_gsm/root_crt.pem,0,1188,10<CR><LF>");
  updateSerial();
...
void setTime(){
  ...
  Serial2.println("AT+HTTPPARA=\"CONTENT\",\"application/json\"");//Activate PDP; Internet connection is available after successful PDP activation
  updateSerial();
  Serial2.println("AT+HTTPPARA=\"URL\",\"https://box-dev.dvlb.ru/server/timeUnix\"");//Get local IP address
  updateSerial();

  // Serial2.println("AT+SSLSETCERT=cert.cer");
  // updateSerial();
