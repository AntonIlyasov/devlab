...
public:
  ...
  void filter_madgwick(){    
    ...
    drone_state.roll_x_from_Madgwick  = filter.getRoll();
    drone_state.pitch_y_from_Madgwick = filter.getPitch();
    drone_state.yaw_z_from_Madgwick   = filter.getYaw();
  }
  ...
  void update_state(float dt){        // dt - [sec]
    ...
    // drone_state.pitch_y_from_ay_rad  = atan2(-drone_state.ax,sqrt(drone_state.ay*drone_state.ay+drone_state.az*drone_state.az));
    ...
    if (millis() - startProgrammTime < 10000) return;   // 10 sec
    calculate_aW();
    ...
    drone_state.dvzW = integrator_az->update(drone_state.azW - 9.81, dt);
...
private:
  ...
  void calculate_aW(){
    ...
    // Serial.println(drone_state.yaw_z_from_az_rad);
