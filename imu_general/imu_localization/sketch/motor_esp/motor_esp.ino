...
void motors_write(int pwm, int time){
  ...
  last_time[2] = 0;
  last_time[3] = 0;
  ledcWrite(M1N, speeds[0]);
  ledcWrite(M1P, speeds[1]);
  ledcWrite(M2N, speeds[2]);
  ledcWrite(M2P, speeds[3]);
  ledcWrite(M3N, speeds[4]);
  ledcWrite(M3P, speeds[5]);
  ledcWrite(M4N, speeds[6]);
  ledcWrite(M4P, speeds[7]);
  delay(time*1000);
}
