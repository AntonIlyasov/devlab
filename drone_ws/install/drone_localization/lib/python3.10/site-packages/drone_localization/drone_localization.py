...
def transform_inverse(rvec, tvec):
    ...
    rotation_matrix_dir = np.array([[cosY * cosZ, sinY * sc - cs, sinY * cc + ss],
                                       [cosY * sinZ, sinY * ss + cc, sinY * cs - sc], 
                                       [-sinY,     cosY * sinX,      cosY * cosX]])
    rotation_matrix_inv = inv(rotation_matrix_dir)
    # print("rotation_matrix_dir:\n", rotation_matrix_dir)
    # print("rotation_matrix_inv:\n", rotation_matrix_inv)
    
    translation_vector_dir = np.array([tvec[0][0][0], tvec[0][1][0], tvec[0][2][0]])
    translation_vector_dir_neg = (-1) * translation_vector_dir
    translation_vector_inv = np.dot(rotation_matrix_inv, translation_vector_dir_neg)
    print("translation_vector_dir:\n", translation_vector_dir)
    print("translation_vector_inv:\n", translation_vector_inv)
    
    alfa = math.atan(rotation_matrix_inv[2][1]/rotation_matrix_inv[2][2])
    beta = math.atan((-1)*rotation_matrix_inv[2][0]/(math.sqrt(1-rotation_matrix_inv[2][0]**2)))
    gamma = math.atan(rotation_matrix_inv[1][0]/rotation_matrix_inv[0][0])
    
    rvec_inv = np.array([alfa, beta, gamma])
    tvec_inv = np.array([translation_vector_inv[0], translation_vector_inv[1], translation_vector_inv[2]])
